"use client";

import { useEffect, useState } from "react";
import { useRouter, useSearchParams } from "next/navigation";
import { facebookService } from "@/services/facebookService";
import { CheckCircle, AlertCircle, Loader2 } from "lucide-react";
import ClientSuspense from "@/components/ClientSuspense";

function FacebookParamsReader({ onParsed }) {
	const searchParams = useSearchParams();

	useEffect(() => {
		const success = searchParams.get("success");
		const error = searchParams.get("error");
		const message = searchParams.get("message");
		onParsed({ success, error, message });
	}, [searchParams, onParsed]);

	return null;
}

export default function FacebookCallback() {
	const router = useRouter();
	const [status, setStatus] = useState("processing"); // processing, success, error
	const [message, setMessage] = useState("");

	const process = ({ success, error, message: msg }) => {
		try {
			// Debug: Log all parameters
			console.log("Callback parameters:", {
				success,
				error,
				message: msg,
			});

			if (error === "true") {
				setStatus("error");
				setMessage(decodeURIComponent(msg) || "Facebook authorization failed");
				return;
			}

			if (success === "true") {
				setStatus("success");
				setMessage("Facebook account connected successfully!");
				setTimeout(() => {
					router.push("/dashboard");
				}, 2000);
			} else {
				setStatus("error");
				setMessage(`Invalid callback parameters. Received: success=${success}, error=${error}, message=${msg}`);
			}
		} catch (err) {
			setStatus("error");
			setMessage("An unexpected error occurred");
		}
	};

	return (
		<div className="min-h-screen bg-gray-900 flex items-center justify-center px-4">
			<ClientSuspense>
				<FacebookParamsReader onParsed={process} />
			</ClientSuspense>
			<div className="bg-gray-800 p-8 rounded-lg border border-gray-700 max-w-md w-full text-center">
				{status === "processing" && (
					<>
						<Loader2 className="h-12 w-12 animate-spin text-blue-500 mx-auto mb-4" />
						<h2 className="text-xl font-semibold text-white mb-2">Connecting to Facebook</h2>
						<p className="text-gray-400">Please wait while we connect your Facebook account...</p>
					</>
				)}

				{status === "success" && (
					<>
						<CheckCircle className="h-12 w-12 text-green-500 mx-auto mb-4" />
						<h2 className="text-xl font-semibold text-white mb-2">Success!</h2>
						<p className="text-gray-400 mb-4">{message}</p>
						<p className="text-sm text-gray-500">Redirecting to dashboard...</p>
					</>
				)}

				{status === "error" && (
					<>
						<AlertCircle className="h-12 w-12 text-red-500 mx-auto mb-4" />
						<h2 className="text-xl font-semibold text-white mb-2">Connection Failed</h2>
						<p className="text-gray-400 mb-4">{message}</p>
						<button
							onClick={() => router.push("/dashboard")}
							className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition"
						>
							Return to Dashboard
						</button>
					</>
				)}
			</div>
		</div>
	);
}
